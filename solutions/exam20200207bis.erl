...
% Further improvement of my sol based on prof's sol (stylistic improvement to be more idiomatic)
...
run_mgr(ReduceF, ReducersMap) ->
    receive
        {reduce, Key, Num} ->
            ...
            case ReducersMap of
                #{Key := ReducerId} ->
                    ReducerId ! {reduce_partial, Num},
                    run_mgr(ReduceF, ReducersMap);
                _ ->
                    RedId = spawn(fun () -> run_reducer(ReduceF, 0, Key) end),
                    RedId ! {reduce_partial, Num},
                    run_mgr(ReduceF, ReducersMap#{Key => RedId})
            end;
        print_results ->
            maps:foreach(
...
run_reducer(ReduceF, Accum, Key) ->
    receive
        {reduce_partial, Num} ->
            run_reducer(ReduceF, ReduceF(Accum, Num), Key);
